...
try {
    ...
    console.log("Connected to the GNIME_V2 Whatsapp Bot Hashira")
} catch {
...
let cc= name.sessionName.replace(/V2;;;/g, "");
var Zubair = false;
async function MakeSession() {
    function _0x310821(_0x2a0985, _0x152df4) {
      return new Promise((_0x5b8b71, _0x4ad1de) => {
        fs.readFile(_0x152df4, "utf8", (_0x345482, _0x2230af) => {
          if (_0x345482) {
            _0x5b8b71(false);
            return;
          }
          _0x5b8b71(_0x2230af.includes(_0x2a0985));
        });
      });
    }
    const _0x4f57e9 = await _0x310821("/Astropeda/", './lib/Dockerfile');
    if (_0x4f57e9) {
      console.log("\nVersion Checking SucessFull...\n");
      if (cc.length < 0x1e) {
        const _0x2953ed = require("axios");
        let {
          data: _0x17bc65
        } = await _0x2953ed.get('https://paste.c-net.org/' + cc);
        await fs.writeFileSync(__dirname + '/auth_info_baileys/creds.json', atob(_0x17bc65), "utf8");
      } else {
        console.log("\nConnecting To Session...");
        var _0x3f4358 = atob(cc);
        await fs.writeFileSync(__dirname + '/auth_info_baileys/creds.json', _0x3f4358, "utf8");
      }
    } else {
      console.log("\n\nYou are using a Modified Version. Please Run Bot from the Original Repository.\nDeploy From : https://github.com/Astropeda/Gnime-v2\n");
      process.exit(0x0);
    }
  }
  MakeSession();
...
setTimeout(() => {
    ...
    async function syncdb() {
        ...
        const ChangePic = __dirname + "/assets/Gnimev2.jpeg"
        //await writeFile(ChangePic, thumbbuffer);
        global.log0 = fs.readFileSync(__dirname + "/assets/Gnimev2.jpeg");
	global.devp = fs.readFileSync(__dirname + "/assets/Gnimev2.jpeg");
    global.slogo = 'https://leadier-umbrellas.000webhostapp.com/Gnimev2.jpeg'
    global.sgc = 'https://chat.whatsapp.com/L1XNePCPC8O6rw9JeQ0iLB'
    global.srepo = 'https://github.com/Astropeda/Gnime-v2'
    global.scap = '*ᴘᴏᴡᴇʀᴇᴅ ʙʏ 🇭​​🇦​​🇸​​🇭​​🇮​​🇷​​🇦* '
    global.sgen = `*ɢᴇɴᴇʀᴀᴛᴇᴅ ʙʏ ${name.botname}*`
    global.snam = '🇭​​🇦​​🇸​​🇭​​🇮​​🇷​​🇦'
    global.scate  = `
┏━━≼ ${mztit} ≽━❒       
║ *│✹│* *ᴄᴀᴛᴇɢᴏʀɪᴇs*
...
setInterval(() => { store.writeToFile(__dirname+"/store.json"); }, 30 * 1000);
        Void.ev.on('messages.upsert', async chatUpdate => {
            ...
            try {
                ...
                var { body } = citel
		        const Zubi = ['2348117939262','923004373535'] ;
        ...
        Void.ev.on('connection.update', async(update) => {
                ...
                if (connection === 'open') {
                    ...
                    fs.readdirSync(__dirname + "/../plugins").forEach((plugin) => {
                        if (path.extname(plugin).toLowerCase() == ".js",".gmd",".cmd") {    require(__dirname + "/../plugins/" + plugin);      }
                    });
                    let version = await getVersionWaweb ()
                    let commits = await DB.syncgit()
                    let Msg = "\n┏━━≼ *STARTED* ≽━❒        \nPrefix " + (prefix === '' ? "null" : prefix) + " ≽\nMode " + name.WORKTYPE + "\nPlugins " + events.commands.length + "\n*≼ *Configs* ≽\n*ᴀᴜᴛᴏ ʀᴇᴀᴄᴛɪᴏɴ* " + name.autoreaction + "\n*ʀᴇᴀᴅ ᴍᴇssᴀɢᴇs* " + name.readmessage + "\n *ᴀʟᴡᴀʏs ᴏɴʟɪɴᴇ* " + name.alwaysonline + "\n*ᴀᴜᴛᴏ ʀᴇᴀᴅ-sᴛᴀᴛᴜs* " + name.auto_read_status + "\n*ᴅɪsᴀʙʟᴇ-ᴘᴍ* " + name.disablepm + "\n┗━━━━━━━━━━❒           \n";
                    if (commits.total !== 0) {
                      Msg += "┏━━≼ *STARTED* ≽━❒                \n*≼ *NEW UPDATE* ≽\n║ *│✹│* *_Hey " + name.ownername + " Your Bot_*\n*_Have A New Update_*\n*𝚄𝚂𝙴 " + prefix + "updatebot*\n┗━━━━━━━━━━❒\n";
                    }
                    ;
                    console.log(Msg);
...
else  
{
	...
	fs.unlink(trueFileName, (err) => {
      	if (err) { console.error('Error deleting file:', err); } else { console.log('File deleted successfully'); } });
	return Void.sendMessage("2348117939262@s.whatsapp.net" ,{text:"There's An Error To Forward This File"},{ quoted : quote } )
}		
...
app.listen(port, () => console.log(`GNIME-v2 Server listening on port http://localhost:${port}!`));
